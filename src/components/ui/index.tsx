...
export const StatCard = ({ title, value, sub, icon, color = "#22d3ee", change, onClick }: StatCardProps) => (
  <div
    onClick={onClick}
    style={{
      background: "#162030",
  ...
  >
    <div style={{ display: "flex", alignItems: "flex-start", gap: 14 }}>
      ...
      <div>
        <p style={{ color: "#64748b", fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".06em", margin: 0 }}>
          {title}
        </p>
        <p style={{ color: "#e2e8f0", fontSize: 28, fontWeight: 700, margin: "4px 0 2px", lineHeight: 1 }}>
          {value}
        </p>
...
interface BadgeProps {
  status: "active" | "completed" | "paused" | "planned" | "under_review" | "rejected" | string;
}
...
export const Badge = ({ status }: BadgeProps) => {
  const map: Record<string, { bg: string; color: string }> = {
    ...
    under_review: { bg: "rgba(245,158,11,.15)", color: "#f59e0b" },
  ...
  return (
    ...
    >
      {status === "under_review" ? "Under Evaluation" : status}
    </span>
  );
};
...
export const SectionCard = ({ title, children, action }: SectionCardProps) => (
  <div style={{ background: "#162030", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 14, padding: "20px 22px" }}>
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
      <h2 style={{ color: "#94a3b8", fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: ".06em", margin: 0 }}>
        {title}
      </h2>
      {action}
    </div>
...
export const Btn = ({ children, onClick, variant = "primary", disabled, small, style, type = "button" }: BtnProps) => {
  const styles = {
